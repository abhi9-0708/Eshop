...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', confirmPassword: '', phone: '', territory: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await register({ name: form.name, email: form.email, password: form.password, phone: form.phone, territory: form.territory });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center auth-gradient px-4 py-8 relative overflow-hidden">
      {/* Decorative blobs */}
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-indigo-200 mt-2 text-sm font-medium">Create your account</p>
        </div>
        ...
        <div className="glass-card rounded-3xl shadow-2xl p-8 border border-white/20">
          ...
          <p className="text-sm text-gray-500 mb-6">Fill in the details to create your account</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Full Name</label>
              <div className="relative">
                <HiOutlineUser className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="input-field pl-11" placeholder="Min 6 chars" minLength="6" required />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <HiOutlineLockClosed className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
                  <input type="password" value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })} className="input-field pl-11" placeholder="Repeat" required />
                </div>
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                <HiOutlinePhone className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
                <input type="text" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="input-field pl-11" placeholder="Optional" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Territory</label>
              <div className="relative">
                <HiOutlineLocationMarker className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
                <input type="text" value={form.territory} onChange={(e) => setForm({ ...form, territory: e.target.value })} className="input-field pl-11" placeholder="Optional" />
              </div>
            </div>

            <button type="submit" disabled={loading} className="btn-primary w-full py-3 text-base font-semibold shadow-lg shadow-primary-600/25 hover:shadow-primary-600/40 transition-all duration-300">
              {loading ? (
